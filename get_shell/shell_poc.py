...
def calc_checksum(frame_bytes: bytes) -> int:
    ...
    ck = 0
    for val in frame_bytes.decode():
        val_int = int(val,16)
        ck = (ck + val_int)&0xff
        
    return ck # 1 byte checksum
...
def build_frame(channel: int, cmd_id: int, payload: bytes = b"") -> bytes:
    ...
    if payload:
        # this is where an overflwo can happen. basically we are sending the size in one nible of how much they play to read, if we send invalid ascii we should be able to get a
        # 0xff
        frame.extend(b"1")
        # encode IP
        prepayload  = encode_le_hex(192, 1).encode()+  encode_le_hex(168, 1).encode() + encode_le_hex(1, 1).encode() + encode_le_hex(10, 1).encode()  + encode_le_hex(1, 1).encode()
        
        frame.extend(encode_be_hex(len(payload) + len(prepayload)//2 , 1).encode())
        frame.extend(prepayload)
        # Payload: raw hex encoding (no endian swap)
        frame.extend(payload.hex().upper().encode())
    else :
...
def cmd_loop(skip_auth: bool):
    ...
    channels = range(0, 7)  # not sure which of these needs to happen
    # update command ID    
